...
func run(args []string) error {
	...
	flags.StringVar(&a.clientID, "client-id", "", "OAuth2 client ID of this application.")
	flags.StringVar(&a.clientSecret, "client-secret", "", "OAuth2 client secret of this application.")
	flags.StringVar(&a.redirectURI, "redirect-uri", "http://127.0.0.1:5555/callback", "Callback URL for OAuth2 responses.")
	flags.StringVar(&issuerURL, "issuer", "", "URL of the OpenID Connect issuer.")
	...
	flags.StringVar(&rootCAs, "issuer-root-ca", "", "Root certificate authorities for the issuer. Defaults to host certs.")
	flags.BoolVar(&debug, "debug", false, "Print all request and responses from the OpenID Connect issuer.")
	err := flags.Parse(args[1:])
	if err != nil {
		return err
	}

	if a.clientID == "" || a.clientSecret == "" || issuerURL == "" {
		return fmt.Errorf("mandatory flags: -issuer, -client-id and -client-secret")
	}
